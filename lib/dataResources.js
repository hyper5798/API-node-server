...
module.exports = {
  ...
  async getDefaultScript(roomId, missionId) {
    ...
    getRoom(room_id) {
      ...
      return new Promise((resolve) => {
        resolve(Room.findOne({
          ...
          attributes: ['id','room_name','pass_time','cp_id']
        }))
      })
    },
    ...
    getScripts(_roomId, _missionId) {
      ...
      return new Promise((resolve) => {
        ...
        resolve(Script.findAll({
          where: _obj,
          attributes: ['id','script_name','mission_id','content','prompt1','prompt2','prompt3','pass','next_pass','next_sequence','note','image_url'] 
        }))
      })
    },
    ...
    getSecurityNodeByMac(mac) {
      const Device = require('../db/models').device
      return new Promise((resolve) => {
        resolve(Device.findOne({
          where: { 
            "macAddr": mac
          },
          attributes: ['id','macAddr','setting_id']
        }))
      })
    },
    createRecord(mobj) {
